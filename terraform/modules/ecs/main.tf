...
======*/
resource "aws_cloudwatch_log_group" "bops" {
  name = "bops"
...
======*/
resource "aws_ecr_repository" "bops_app" {
...
resource "aws_alb" "alb_bops" {
  name            = "${var.environment}-alb-bops"
...
resource "aws_alb_listener" "bops" {
  load_balancer_arn = aws_alb.alb_bops.arn
  port              = "80"
  protocol          = "HTTP"
...
#   alarm_name          = "${var.environment}_bops_web_cpu_utilization_high"
