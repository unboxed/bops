...
class MigratePolicyInformation < ActiveRecord::Migration[7.1]
  def up
    ...
    filepath = Rails.root.join("config/locales/references.yml")

    if File.exist?(filepath)
      data = YAML.load_file(filepath)

      data["en"]["schedules"].first["parts"].each do |part_key, part_data|
        part = schedule.policy_parts.create!(
          number: part_key,
          name: part_data["name"]
        )

        part_data["classes"].each do |class_data|
          new_policy_class = part.new_policy_classes.create!(
          ...
          class_data["policies_attributes"].each do |section_data|
            new_policy_class.policy_sections.create!(
