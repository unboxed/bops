  ...
  describe "#update_all" do
    context "when the hash contains blank numbers" do
      let(:drawing_numbers_hash) do
        {
          drawings_to_update.first.id => {
            "numbers": "one, two"
          },
          drawings_to_update.last.id => {
            "numbers": ""
          }
        }
      end
      ...
      it "populates the errors object with a relevant message" do
        subject.update_all

        expect(subject.errors).not_to be_empty
        expect(subject.errors[:base]).to include "Supply at least one number for each drawing"
      end
