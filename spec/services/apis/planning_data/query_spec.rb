...
RSpec.describe Apis::PlanningData::Query do
  ...
  describe "#get_entity" do
    let(:submission) { JSON.parse(Rails.root.join("spec/fixtures/files/entities/1000005.json").read, symbolize_names: true) }
    ...
    context "when the request is successful" do
      context "when a valid entity reference is supplied" do
        ...
        it "returns entity's data" do
          expect(query.get_entity("1000005")).to eq(submission)
        end
      end
    end
  end
