...
RSpec.describe "Add and assign neighbours task", :capybara, type: :system do
  let(:local_authority) { create(:local_authority, :default) }
  let(:assessor) { create(:user, :assessor, local_authority: local_authority) }
  let(:application_type) { create(:application_type, :prior_approval) }
  let(:planning_application) { create(:planning_application, :from_planx_prior_approval, :with_boundary_geojson, :published, local_authority:, application_type:, uprn:) }
  let(:uprn) { "20000111111" }
  let(:consultation) { planning_application.consultation }
  let(:slug) { "consultees-neighbours-and-publicity/neighbours/add-and-assign-neighbours" }
  ...
  context "when drawing a polygon to search for addresses" do
    ...
    it "shows the relevant content for searching by polygon" do
      ...
      page.find("summary", text: /View more instructions/).click
    ...
    it "I can remove an address before adding the relevant neighbour addresses" do
      ...
      expect(consultation.neighbours.pluck(:address)).to eq(["5, COXSON WAY, LONDON, SE1 2XB"])

      click_button "Save changes"
    end

    it "I can view the previously drawn area", pending: "why does this fail?" do
      map = find("my-map")
