...
RSpec.describe "Add conditions task", type: :system do
  ...
  it "can edit a condition and fields are pre-populated" do
    within :sidebar do
      click_link "Add conditions"
    end

    condition = planning_application.condition_set.conditions.first

    within("#conditions-list") do
      first(:link, "Edit").click
    end

    expect(page).to have_field("Enter condition", with: condition.text)
    expect(page).to have_field("Enter a reason for this condition", with: condition.reason)
  ...
  it "can delete a condition", capybara: true do
    within :sidebar do
      click_link "Add conditions"
    end
    ...
    expect(page).to have_content("Condition successfully removed")
