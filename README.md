...
[BOPS infrastructure repo](https://github.com/unboxed/bops-terraform/) | [FAQs](docs/FAQs.md) | [BOPS Applicants](https://github.com/unboxed/bops-applicants/)

## Back-Office Planning System (BOPS)

This README is aimed at developers wishing to work on this project or learn more about the code base in this repo. If you need technical guidance for getting set up on BOPS, contact us [here](mailto:bops-team@unboxedconsulting.com).

## Getting started

### Using Docker

The easiest way to run the app is with Docker. First `cd` into the `bops-applicants` folder and run:
...
Once your containers are running, you can use the Makefile to get a prompt and set up your database:
...
You will also need to have PostgreSQL installed and the PostGIS extension enabled.
...
The staging and production environments need keys generated by [GOV.UK Notify](https://www.notifications.service.gov.uk/sign-in), which is a government service that enables the bulk secure sending of emails, SMS and letters. To run the app locally, you will need an environment variable set to `NOTIFY_API_KEY` which should contain a mock value.
...
You can find this value in the AWS Parameter Store
...
Once you have the application running, you can submit planning application through the API.

API documentation is available at /api/docs/index.html.

You can do this through the provided Swagger documentation at /api/docs/index.html
