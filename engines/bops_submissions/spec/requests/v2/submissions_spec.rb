...
RSpec.describe "BOPS Submissions API", type: :request do
  ...
  before do
    create(:api_user, permissions: %w[planning_application:submit], token: "bops_EjWSP1javBbvZFtRYiWs6y5orH4R748qapSGLNZsJw", local_authority:)
  end
      ...
      response "200", "submission accepted" do
        ...
        before do
          stub_request(:get, event[:documentLinks].first[:documentLink])
            .to_return(
              status: 200,
              body: BopsSubmissions::Engine.root.join("spec", "fixtures", "files", "PT-10087984.zip").read,
