...
RSpec.describe "Check constraints task", type: :system do
  ...
  [:planning_permission, :pre_application].each do |application_type|
    context "for a #{application_type} case" do
      let(:planning_application) { create(:planning_application, application_type, :not_started, :with_constraints, local_authority:, api_user:) }
