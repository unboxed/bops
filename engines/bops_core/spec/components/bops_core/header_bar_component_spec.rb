...
RSpec.describe BopsCore::HeaderBarComponent, type: :component do
  ...
  it "renders left items inside a horizontal list with no divider elements" do
    ...
    expect(page).to have_css("ul.bops-header-bar__list")
    expect(page).to have_css("ul.bops-header-bar__list > li.bops-header-bar__item", count: left_items.size)
  end

  context "with minimal inputs" do
    it "handles empty right items" do
      render_component(right: [])
      expect(page).to have_css(".bops-header-bar__list--right")
    ...
    it "puts single left item into an array" do
      ...
      expect(page).to have_css(".bops-header-bar__list .bops-header-bar__item", text: "Only item")
    end
  end
  ...
  context "when a toggle is provided" do
    ...
    it "renders a toggle button and hidden panel" do
      ...
      toggle_button = page.find(".bops-header-bar__item--toggle button", visible: :all)
