...
module Tasks
  class ReviewDocumentsForm < Form
    self.task_actions = %w[save_draft save_and_complete edit_form save_document]
    ...
    # The generic :id param is treated as document_id for this task
    def document
    ...
    def url(options = {})
      if params[:id].present?
        # When editing a specific document, include the ID in the URL
        planning_application_edit_task_component_path(
    ...
    def save_document
      transaction do
        ...
        # If marked as invalid, create a replacement document validation request
        if document_invalid?
