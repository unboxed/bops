...
module Tasks
  class Form
    ...
    # Dynamic route helper that maps route names to the main app's nested route helpers
    # e.g., route_for(:task, ...) calls planning_application_task_path(...)
    def route_for(route_name, *args, **options)
      helper_name = :"planning_application_#{route_name}_path"
      send(helper_name, *args, **options)
    end
