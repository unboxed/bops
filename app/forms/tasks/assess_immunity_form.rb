...
module Tasks
  class AssessImmunityForm < Form
    ...
    after_initialize do
      @assessment_detail = planning_application.assessment_details.find_or_initialize_by(category: "assess_immunity")
      @rejected_assessment_detail = planning_application.rejected_assessment_detail(category: "assess_immunity")
    end

    attr_reader :assessment_detail, :rejected_assessment_detail

    with_options on: %i[save_and_complete save_draft] do
      validates :entry, presence: {message: "Immunity assessment cannot be blank"}
    end
