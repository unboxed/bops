...
class PlanningApplication < ApplicationRecord
  ...
  scope :for_user, ->(user_id) { joins(:case_record).where(case_record: {user_id: user_id}) }
  ...
  def pre_commencement_condition_set
    super || create_pre_commencement_condition_set!
  end
  ...
  def consultee_responses_closed?
    ...
    consultation_required == false || consultation.complete? || consultation.end_date&.past?
  end
